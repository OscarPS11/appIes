...
export class CalendarioProvider {
  ...
  createNewEvent(event: any) {
    let modalNewEvent = this.modalCtrl.create(ModalPage, { 'date': event.date, 'month': event.month, 'year': event.year, 'eventos': this.calendario.eventos }, { enableBackdropDismiss: false });
